package transition9.serialization;
...
class Serialization
{
	...
	public static function encode (bytes :haxe.io.BytesData) :String
	{
		#if flash
		//It's fast.  Polygonal?
	    return null;//jpauclair.Base64.encode(bytes);
	    #else
	...
	public static function decode (base64Encoded :String) :haxe.io.BytesData
	{
		#if flash
		//It's fast.  Polygonal?
	    return null;//jpauclair.Base64.decode(base64Encoded);
	    #else
	    throw "What is the Base64 encoder for this platform?";
	    return null;
	    #end
	...
	static function deserClass(o, resolved :Class<Dynamic>, ?newObj :Dynamic) :Dynamic 
	{
		...
		// transition9.util.Assert.isNotNull(resolved, " resolved is null");
		...
		if (rtti == null) {
			// Log.error(Type.getClassName(resolved) + " has no rtti info");
		...
		switch(el) {
								...
								if (i == null) {
									...
									switch(st) {
										...
										case CEnum(enumPath,_) :
											...
											transition9.util.Assert.isNotNull(i);
			...
			case CEnum(enumPath, params) :
				if (enumPath == "Bool") {
					...
					var e = Type.resolveEnum(enumPath);
					transition9.util.Assert.isNotNull(e, ' e is null');
					transition9.util.Assert.isNotNull(val, ' val is null');
					...
					if (fieldMeta != null && !Reflect.isFunction(Reflect.field(obj, fieldName)) && !MetaUtil.isFieldMetaData(cls, fieldName, "ignore")) {
						...
						switch(fieldMeta.type) {
							...
							case CEnum(enumPath, _):
								var e = Type.resolveEnum(enumPath);
								transition9.util.Assert.isNotNull(e, ' e is null');
								transition9.util.Assert.isNotNull(fieldStringVal, ' fieldStringVal is null');
