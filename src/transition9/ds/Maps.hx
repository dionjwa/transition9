...
class Maps
{
     ...
     */
    @:macro
    public static function testBuild (const :Expr) :Expr
    {
        // Rewrites self.get(ComponentClass) to ComponentClass.getFrom(self)
        #if macro
		// import haxe.macro.Expr;
		haxe.macro.Context.warning(const + "", haxe.macro.Context.currentPos());
		...
		switch(const.expr ) {
			case EConst(c):
				switch (c) {
					...
					case CFloat(s):
					default: //Cannot parse type
						haxe.macro.Context.warning("Cannot parse map type: " + c, haxe.macro.Context.currentPos());
				}
			default: //Ignored
		}
	...
	public static function newHashMap <K, V> (type :ValueType) :Map<K, V>
	{
		...
		transition9.util.Assert.isNotNull(type);
		switch (type) {
		...
		// 	// Log.warn("Returning StringMap because: null from Type.typeof(" + Std.string(keyClazz) + ")"); 
		// 	return new StringMap<K, V>();
		// } else {
		...
		// if (classname == "transition9.ds.Hashable") {
		...
		// 	// 	return new transition9.ds.maps.KeyedObjectMap();
		// 	// } else {
		// 	// 	Log.warn("keyClazz=" + Type.getClassName(keyClazz) + ", JS target doesn't support non-String/Int dictionaries, and the key is not Hashable, nor does it have a key:Int field. Falling back to a StringMap, where the objects are converted to a String with transition9.util.StringUtil.getStringKey()");
		...
		// 	// #if cpp
		// 	// if (transition9.rtti.ReflectUtil.is(k, "transition9.util.Enumerable")) {
		// 	// #else
		// 	if (Std.is(k, transition9.util.Enumerable)) {
		// 	// #end
		...
		// 	} 
		// 	else if (Std.is(k, transition9.ds.Hashable)) {
		// 		untyped return new transition9.ds.maps.PolygonalHashableMap<K, V>();
		// 	}
		...
		// 			return new transition9.ds.maps.KeyedObjectMap();
		// 		} else {
		// 			Log.warn("keyClazz=" + Type.getClassName(keyClazz) + ", target doesn't support non-String/Int dictionaries, and the key is not Hashable, nor does it have a key:Int field. Falling back to a StringMap, where the objects are converted to a String with transition9.util.StringUtil.getStringKey()");
		// 			return new StringMap();
		// 		}
		// 	}
		// }
	...
	static function createMapFromClass <K, V>(cls :Class<Dynamic>) :Map<K, V>
	{
		...
		if (Std.is(k, Hashable)) {
			...
			if (IS_OBJECT_DICTIONARY) {
				...
				trace("cls=" + classname + ", target doesn't support non-String/Int dictionaries, and the key is not Hashable, nor does it have a key:Int field. Falling back to a StringMap, where the objects are converted to a String with transition9.util.StringUtil.getStringKey()");
				return new StringMap();
			}
		}
	}
