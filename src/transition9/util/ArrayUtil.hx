...
class ArrayUtil	
{
	public static function iteratorIgnoringNulls<T> (arr :Array<T>) :Iterator<T>
	{
		return new ArrayIteratorIgnoringNulls(arr);
	}
	...
	{
		return transition9.util.IterUtil.indexOf(it, element);
	}
	...
	public static function join (arr :Array<Dynamic>) :String
	{
		...
		sb.add("[");
		for (obj in arr) {
			sb.add(obj + "::" + transition9.rtti.ReflectUtil.getClassName(obj) + ", ");
		}
		sb.add("]");
		return sb.toString();
	}
...
class ArrayIteratorIgnoringNulls<T>
{
	var _arr :Array<T>;
	var _index :Int;
	...
	**/
	...
	{
		if (_arr == null || _index >= _arr.length) {
			return false;
		}
