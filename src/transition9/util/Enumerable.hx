...
using transition9.util.IterUtil;
	...
	implements Hashable, implements Equalable<Enumerable<T>>, implements Comparable<Enumerable<T>> 
{
	...
	public static function createParser <T> (cls :Class<T>) :Xml->T
	{
		return function (xml :Xml) :T {
			transition9.util.Assert.isTrue(xml.firstChild() != null && xml.firstChild().nodeValue != null);
	...
	public static  function serializeEnumerable (xml :Xml, cls :Class<Dynamic>) :Void
	{
		...
		transition9.util.Assert.isNotNull(_enums.get(cls), "No Enumerable of type " + cls);
		for (e in _enums.get(cls)) {
	...
	{
		transition9.util.Assert.isNotNull(_enums);
		transition9.util.Assert.isNotNull(xml);
		transition9.util.Assert.isNotNull(xml.get("class"));
		var cls = Type.resolveClass(xml.get("class"));
		transition9.util.Assert.isNotNull(cls, "Resolved class is null from 'class' attribute=" + xml.get("class"));
		transition9.util.Assert.isNotNull(_enums.get(cls), "No Enumerable of type " + cls);
		var enums = _enums.get(cls);
		for (childXML in xml) {
			if (childXML.nodeType == Xml.Element) {
				var e = enums.get(childXML.nodeName);
				transition9.util.Assert.isNotNull(e, "No Enumerable of type " + cls + " with name=" + childXML.nodeName);
	...
	static function deserializeAllFromEmbeddedXML () :Void
	{
		for (enumCls in _enums.keys()) {
			if (haxe.Resource.listNames().has(Type.getClassName(enumCls))) {
				Log.info("Deserializing  " + Type.getClassName(enumCls));
	...
	public function serialize (xml :Xml) :Void
	{
		...
		var root = Xml.parse(haxe.Resource.getString(transition9.rtti.ReflectUtil.getClassName(this)).trim()).firstChild();
	...
	{
		// transition9.util.Assert.isTrue(Std.is(other, Type.getClass(this)), "Not same Enumerable type");
