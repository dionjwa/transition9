	...
	implements Hashable, implements Equalable<Enumerable<T>>, implements Comparable<Enumerable<T>> 
{
	...
	public static function createParser <T> (cls :Class<T>) :Xml->T
	{
		return function (xml :Xml) :T {
			org.transition9.util.Assert.isTrue(xml.firstChild() != null && xml.firstChild().nodeValue != null);
	...
	public static  function serializeEnumerable (xml :Xml, cls :Class<Dynamic>) :Void
	{
		...
		org.transition9.util.Assert.isNotNull(_enums.get(cls), "No Enumerable of type " + cls);
		for (e in _enums.get(cls)) {
	...
	{
		org.transition9.util.Assert.isNotNull(_enums);
		org.transition9.util.Assert.isNotNull(xml);
		org.transition9.util.Assert.isNotNull(xml.get("class"));
		var cls = Type.resolveClass(xml.get("class"));
		org.transition9.util.Assert.isNotNull(cls, "Resolved class is null from 'class' attribute=" + xml.get("class"));
		org.transition9.util.Assert.isNotNull(_enums.get(cls), "No Enumerable of type " + cls);
		var enums = _enums.get(cls);
		for (childXML in xml) {
			if (childXML.nodeType == Xml.Element) {
				var e = enums.get(childXML.nodeName);
				org.transition9.util.Assert.isNotNull(e, "No Enumerable of type " + cls + " with name=" + childXML.nodeName);
	...
	static function deserializeAllFromEmbeddedXML () :Void
	{
		for (enumCls in _enums.keys()) {
			if (haxe.Resource.listNames().has(Type.getClassName(enumCls))) {
				org.transition9.util.Log.debug("Deserializing  " + Type.getClassName(enumCls));
	...
	public function serialize (xml :Xml) :Void
	{
		...
		var root = Xml.parse(haxe.Resource.getString(org.transition9.rtti.ReflectUtil.getClassName(this)).trim()).firstChild();
	...
	{
		// org.transition9.util.Assert.isTrue(Std.is(other, Type.getClass(this)), "Not same Enumerable type");
