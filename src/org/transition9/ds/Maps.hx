...
import org.transition9.ds.maps.EnumMap;
import org.transition9.ds.maps.HashMap;
import org.transition9.ds.maps.HashableMap;
import org.transition9.ds.maps.IntHashMap;
import org.transition9.ds.maps.StringMap;
import org.transition9.ds.maps.TransformKeyMap;
...
class Maps
{
	...
	public static function newHashMap <K, V> (type :ValueType) :Map<K, V>
	{
		...
		// 	// org.transition9.util.Log.warn("Returning StringMap because: null from Type.typeof(" + Std.string(keyClazz) + ")"); 
		// 	return new StringMap<K, V>();
		// } else {
		...
		// if (classname == "org.transition9.ds.Hashable") {
		...
		// 	// 	return new org.transition9.ds.maps.KeyedObjectMap();
		// 	// } else {
		// 	// 	org.transition9.util.Log.warn("keyClazz=" + Type.getClassName(keyClazz) + ", JS target doesn't support non-String/Int dictionaries, and the key is not Hashable, nor does it have a key:Int field. Falling back to a StringMap, where the objects are converted to a String with org.transition9.util.StringUtil.getStringKey()");
		...
		// 	// #if cpp
		// 	// if (org.transition9.rtti.ReflectUtil.is(k, "org.transition9.util.Enumerable")) {
		// 	// #else
		// 	if (Std.is(k, org.transition9.util.Enumerable)) {
		// 	// #end
		...
		// 			return new org.transition9.ds.maps.KeyedObjectMap();
		// 		} else {
		// 			org.transition9.util.Log.warn("keyClazz=" + Type.getClassName(keyClazz) + ", target doesn't support non-String/Int dictionaries, and the key is not Hashable, nor does it have a key:Int field. Falling back to a StringMap, where the objects are converted to a String with org.transition9.util.StringUtil.getStringKey()");
		// 			return new StringMap();
		// 		}
		// 	}
		// }
	...
	static function createMapFromClass <K, V>(cls :Class<Dynamic>) :Map<K, V>
	{
		...
		if (Std.is(k, org.transition9.util.Enumerable)) {
			...
			if (IS_OBJECT_DICTIONARY) {
				...
				org.transition9.util.Log.warn("cls=" + classname + ", target doesn't support non-String/Int dictionaries, and the key is not Hashable, nor does it have a key:Int field. Falling back to a StringMap, where the objects are converted to a String with org.transition9.util.StringUtil.getStringKey()");
				return new StringMap();
			}
		}
	}
